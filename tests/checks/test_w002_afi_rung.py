from l5x_lint.checks.w002_afi_rung import w002_afi_rung
...
def test_afi_first_instruction():
    ...
    loc = _loc()
    result = w002_afi_rung(r, table, loc)
    assert len(result) == 1
    d = result[0]
    assert d.code == "W002"
...
def test_afi_via_analyze_pipeline():
    _reset_registry()
    analyze.register(w002_afi_rung)
    ...
    assert ar.diagnostics[0].code == "W002"
