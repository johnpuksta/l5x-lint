...
from l5x_lint.domain.models import Controller, Location, Routine, TagPath, TagPathSegment
from l5x_lint.domain.rll_models import Instruction, Operand, ParsedRung
from l5x_lint.domain.st_models import StAssignment, StBinaryOp, StLiteral, StProgram, StTagRef
    ...
    prog = StProgram(statements=[
        StAssignment(
            ...
            expression=StBinaryOp(
                left=StBinaryOp(
                    left=StTagRef(path=TagPath(segments=[TagPathSegment(name="r")])),
                    op="+",
...
def test_rll_equ_with_float_emits_ws101():
    r = Routine(
        ...
        rll_rungs=[ParsedRung(
            number=0, text="",
            instructions=[Instruction(opcode="EQU", operands=[Operand(value="Motor_Speed"), Operand(value="100.5")])],
        )],
    )
    c = Controller(name="Test")
    table = build_symbol_table(c)
...
def test_rll_grt_no_float_no_diagnostic():
    r = Routine(
        ...
        rll_rungs=[ParsedRung(
            number=0, text="",
            instructions=[Instruction(opcode="GRT", operands=[Operand(value="Speed"), Operand(value="100")])],
        )],
    )
    c = Controller(name="Test")
    table = build_symbol_table(c)
