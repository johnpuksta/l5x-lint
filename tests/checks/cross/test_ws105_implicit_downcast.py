...
def test_lint_to_dint_downcast_emits_ws105():
    prog = StProgram(statements=[
        StAssignment(
            ...
            expression=StTagRef(path=TagPath(segments=[TagPathSegment(name="wide")])),
        ),
    ])
    ...
    c = Controller(
        ...
        data_types=[DataType(name="DINT", family="", class_=""), DataType(name="LINT", family="", class_="")],
...
def test_dint_to_lint_widen_no_diagnostic():
    prog = StProgram(statements=[
        StAssignment(
            ...
            expression=StTagRef(path=TagPath(segments=[TagPathSegment(name="narrow")])),
        ),
    ])
    r = Routine(name="Main", type="ST", st_body=prog)
    c = Controller(
