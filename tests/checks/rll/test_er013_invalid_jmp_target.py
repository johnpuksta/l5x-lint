from l5x_lint.checks.rll.er013_invalid_jmp_target import er013_invalid_jmp_target
...
def test_jmp_to_existing_label_no_diagnostic():
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(1, [Instruction(opcode="LBL", operands=[Operand(value="Mark")])]),
        _make_rung(2, [Instruction(opcode="JMP", operands=[Operand(value="Mark")])]),
    ])
    diags = er013_invalid_jmp_target(r, SymbolTable(), Location(program="P", routine="Test"))
    assert diags == []


def test_jmp_to_nonexistent_label_emits_er013():
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(1, [Instruction(opcode="JMP", operands=[Operand(value="GoneLabel")])]),
    ])
    diags = er013_invalid_jmp_target(r, SymbolTable(), Location(program="P", routine="Test"))
    assert len(diags) == 1
    ...
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(1, [
            Instruction(opcode="XIC", operands=[Operand(value="A")],
                        branch=[[Instruction(opcode="JMP", operands=[Operand(value="Missing")])]]),
        ]),
    ])
    diags = er013_invalid_jmp_target(r, SymbolTable(), Location(program="P", routine="Test"))
    assert len(diags) == 1
