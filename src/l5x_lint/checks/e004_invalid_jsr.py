...
from l5x_lint.domain.errors import E004
                ...
                if inst.opcode.upper() in ("JSR", "JXR"):
                    if inst.operands and inst.operands[0].value not in valid_routines:
                        result.append(
                            Diagnostic(
                                code=E004.code,
                                severity=E004.severity,
                                location=Location(
                                    program=loc.program, routine=loc.routine, rung=rung.number  # noqa: E501
                                ),
                                message=E004(routine=inst.operands[0].value).message,
                            )
                        )
                ...
                if stmt.routine_name not in valid_routines:
                    result.append(
                        Diagnostic(
                            ...
                            severity=E004.severity,
                            location=loc,
                            message=E004(routine=stmt.routine_name).message,
                        )
                    )
