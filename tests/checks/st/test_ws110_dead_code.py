...
from l5x_lint.domain.st_models import StAssignment, StLiteral, StProgram, StReturn, StTagRef
    ...
    r = Routine(name="Test", type="ST",
                st_body=StProgram(statements=[
                    ...
                    StAssignment(
                        ...
                        expression=StLiteral(value=5), line=5,
                    ),
                ]))
    diags = ws110_dead_code(r, SymbolTable(), Location(program="P", routine="Test"))
    assert len(diags) == 1
    ...
    r = Routine(name="Test", type="ST",
                st_body=StProgram(statements=[
                    StAssignment(
                        ...
                        expression=StLiteral(value=1), line=1,
                    ),
                    StReturn(line=2),
                    StAssignment(
                        target=TagPath(segments=[TagPathSegment(name="b")]),
                        expression=StLiteral(value=2), line=3,
                    ),
                    StAssignment(
                        target=TagPath(segments=[TagPathSegment(name="c")]),
                        expression=StLiteral(value=3), line=4,
                    ),
                ]))
    diags = ws110_dead_code(r, SymbolTable(), Location(program="P", routine="Test"))
    assert len(diags) == 2
