...
from l5x_lint.checks._codes import WS102
        ...
        for i, line in enumerate(routine.cdata.splitlines(), 1):
            ...
            if _ZERO_DIV_RE.search(stripped) or _ZERO_MOD_RE.search(stripped):
                result.append(Diagnostic(
                    code=WS102.code, severity=WS102.severity,
                    location=Location(program=loc.program, routine=loc.routine, line=i),
            ...
            for inst in rung.instructions:
                if inst.opcode.upper() in ("DIV", "MOD", "CPT"):
                    for op in inst.operands:
                        if _ZERO_DIV_RE.search(op.value) or _ZERO_MOD_RE.search(op.value):
                            result.append(Diagnostic(
                                code=WS102.code, severity=WS102.severity,
                                location=Location(
                                ...
                                message=WS102(text=f"{inst.opcode}({op.value})").message,
                            ))
    return result
