from l5x_lint.checks._codes import EC001
from l5x_lint.checks.cross.ec001_undefined_tag import (
    _check_rll,
    _check_st,
    _extract_base,
    ec001_undefined_tag,
...
def test_rll_unknown_tag_emits_ec001():
    ...
    assert d.message == EC001(name="NoSuchTag").message
...
def test_rll_mixed_known_unknown():
    ...
    result = _check_rll(r, table, loc)
    assert len(result) == 1
    assert result[0].message == EC001(name="BadTag").message
...
def test_rll_jsr_unknown_param_emits_ec001():
    jsr = Instruction(
        opcode="JSR",
    ...
    result = _check_rll(r, table, loc)
    assert len(result) == 1
    assert result[0].message == EC001(name="MissingParam").message
...
def test_rll_branches_checked():
    ...
    result = _check_rll(r, table, loc)
    assert len(result) == 1
    assert result[0].message == EC001(name="NoTag").message
...
def test_rll_multiple_rungs():
    ...
    result = _check_rll(r, table, loc)
    assert len(result) == 1
    assert result[0].message == EC001(name="Missing").message
...
def test_st_unknown_tag_emits_ec001():
    ...
    result = _check_st(r, table, loc)
    assert len(result) == 1
    assert result[0].message == EC001(name="MissingIn").message
...
def test_st_target_unknown():
    ...
    result = _check_st(r, table, loc)
    assert len(result) == 1
    assert result[0].message == EC001(name="UndefTarget").message
...
def test_st_binary_op_tags():
    ...
    result = _check_st(r, table, loc)
    assert len(result) == 1
    assert result[0].message == EC001(name="MissingB").message
...
def test_ec001_via_analyze_pipeline():
    _reset_registry()
    ...
    assert ar.diagnostics[0].code == "EC001"


def test_ec001_no_duplicates_for_same_tag():
    _reset_registry()
    analyze.register(ec001_undefined_tag)
    rung = ParsedRung(
        number=0,
