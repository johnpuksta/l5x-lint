from l5x_lint.checks.st.ws109_for_var_assign import ws109_for_var_assign
from l5x_lint.domain.models import Location, Routine, TagPath, TagPathSegment
from l5x_lint.domain.st_models import StAssignment, StFor, StLiteral, StProgram, StTagRef
...
def test_for_var_not_assigned_no_diagnostic():
    r = Routine(name="Test", type="ST",
                st_body=StProgram(statements=[
                    StFor(variable=TagPath(segments=[TagPathSegment(name="i")]),
                          start=StLiteral(value=1), end=StLiteral(value=10),
                          body=[StAssignment(
                              target=TagPath(segments=[TagPathSegment(name="x")]),
                              expression=StLiteral(value=1),
                          )]),
                ]))
    diags = ws109_for_var_assign(r, SymbolTable(), Location(program="P", routine="Test"))
    assert diags == []
                ...
                st_body=StProgram(statements=[
                    StFor(variable=TagPath(segments=[TagPathSegment(name="i")]),
                          ...
                          body=[StAssignment(
                              ...
                              expression=StLiteral(value=5), line=7,
                          )]),
                ]))
    diags = ws109_for_var_assign(r, SymbolTable(), Location(program="P", routine="Test"))
    assert len(diags) == 1
