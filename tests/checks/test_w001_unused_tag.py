from l5x_lint.checks.w001_unused_tag import _reset, w001_unused_tag
...
def test_used_controller_tag_no_diagnostic():
    ...
    result = w001_unused_tag(r, table, _loc())
    assert result == []


def test_unused_controller_tag_emits_w001():
    _reset()
...
def test_used_program_tag_no_diagnostic():
    ...
    table = build_symbol_table(c)
    result = w001_unused_tag(r, table, _loc(program="Prog"))
    assert result == []
