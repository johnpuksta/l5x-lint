from l5x_lint.domain.checks._codes import WR003
from l5x_lint.domain.checks.opcodes import INPUT_OPCODES, OUTPUT_OPCODES
    ...
    for name in inputs:
        if name not in outputs:
            result.append(
                Diagnostic(
                    code=WR003.code,
                    severity=WR003.severity,
                    location=loc,
                    message=WR003(name=name).message,
                )
            )
    return result
