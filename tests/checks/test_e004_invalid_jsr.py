from l5x_lint.checks.e004_invalid_jsr import e004_invalid_jsr
...
def test_valid_jsr_no_diagnostic():
    ...
    result = e004_invalid_jsr(r, table, _loc())
    assert result == []


def test_invalid_jsr_emits_e004():
    r = Routine(name="Main", type="RLL", rll_rungs=[_rung(_inst("JSR", "NoSuch"))])
    c = Controller(name="Test")
    table = build_symbol_table(c)
    result = e004_invalid_jsr(r, table, _loc())
    assert len(result) == 1
    assert result[0].code == "E004"
