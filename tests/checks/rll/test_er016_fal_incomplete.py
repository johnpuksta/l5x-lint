    ...
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(1, [
            Instruction(opcode="FAL", operands=[
                Operand(value="INC"), Operand(value="MyCtrl"), Operand(value="MyDest"),
            ]),
        ]),
    ])
    diags = er016_fal_incomplete(r, SymbolTable(), Location(program="P", routine="Test"))
    assert diags == []
...
def test_fsc_two_operands_emits_er016():
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(4, [
            Instruction(opcode="FSC", operands=[
                Operand(value="ALL"), Operand(value="MyCtrl"),
            ]),
        ]),
    ])
    diags = er016_fal_incomplete(r, SymbolTable(), Location(program="P", routine="Test"))
    ...
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(6, [
            Instruction(opcode="XIC", operands=[], branch=[
                [Instruction(
                    opcode="FAL",
                    operands=[Operand("ALL"), Operand("Ctrl"), Operand("Dest")],
                )],
            ]),
        ]),
    ])
