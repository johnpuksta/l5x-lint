from l5x_lint.checks.e010_cross_scope import e010_cross_scope
...
def test_same_program_no_diagnostic():
    ...
    result = e010_cross_scope(r, table, _loc(program="Prog"))
    assert result == []
...
def test_controller_tag_cross_program_no_diagnostic():
    ...
    table = build_symbol_table(c)
    result = e010_cross_scope(r, table, _loc(program="ProgA"))
    assert result == []


def test_different_program_tag_emits_e010():
    ...
    result = e010_cross_scope(r, table, loc)
    assert len(result) == 1
    assert result[0].code == "E010"
...
def test_non_rll_ignored():
    ...
    c = Controller(name="Test")
    table = build_symbol_table(c)
    result = e010_cross_scope(r, table, _loc())
    assert result == []
