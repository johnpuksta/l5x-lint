...
def test_gsv_known_class_no_diagnostic():
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(1, [
            Instruction(opcode="GSV", operands=[
                Operand(value="Program"), Operand(value="MyProg"),
                Operand(value="Name"),
            ]),
        ]),
    ])
    diags = wr009_gsv_invalid_class(r, SymbolTable(), Location(program="P", routine="Test"))
    assert diags == []
...
def test_gsv_invalid_class_emits_wr009():
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(2, [
            Instruction(opcode="GSV", operands=[
                Operand(value="InvalidClass"), Operand(value="Obj"),
                Operand(value="Attr"),
            ]),
        ]),
    ])
...
def test_ssv_invalid_class_emits_wr009():
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(3, [
            Instruction(opcode="SSV", operands=[
                Operand(value="FooBar"), Operand(value="Obj"),
                Operand(value="Attr"),
            ]),
        ]),
    ])
...
def test_gsv_controller_class_no_diagnostic():
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(1, [
            Instruction(opcode="GSV", operands=[
                Operand(value="Controller"), Operand(value=""),
                Operand(value="Name"),
            ]),
        ]),
    ])
...
def test_gsv_module_class_no_diagnostic():
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(1, [
            Instruction(opcode="GSV", operands=[
                Operand(value="Module"), Operand(value="Local"),
    ...
    r = Routine(name="Test", type="RLL", rll_rungs=[
        _make_rung(4, [
            Instruction(opcode="XIC", operands=[], branch=[
                [Instruction(
                    opcode="GSV",
                    operands=[Operand("BadClass"), Operand("O"), Operand("A")],
                )],
            ]),
        ]),
    ])
