...
from l5x_lint.domain.errors import E003
            ...
            if isinstance(stmt, StCall):
                ...
                if name not in _BUILTIN_OPCODES and stmt.name not in symbols.aoi_names:
                    result.append(
                        Diagnostic(
                            code=E003.code,
                            severity=E003.severity,
                            location=loc,
                            message=E003(name=stmt.name).message,
                        )
                    )
        ...
        if opcode not in _BUILTIN_OPCODES and inst.opcode not in symbols.aoi_names:
            result.append(
                Diagnostic(
                    ...
                    message=E003(name=inst.opcode).message,
                )
            )
        if inst.branch:
