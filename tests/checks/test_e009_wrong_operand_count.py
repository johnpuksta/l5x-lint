from l5x_lint.checks.e009_wrong_operand_count import e009_wrong_operand_count
...
def test_xic_one_operand_no_diagnostic():
    ...
    result = e009_wrong_operand_count(r, table, _loc())
    assert result == []


def test_xic_zero_operands_emits_e009():
    r = Routine(name="Main", type="RLL", rll_rungs=[_rung(_inst("XIC"))])
    c = Controller(name="Test")
    table = build_symbol_table(c)
    result = e009_wrong_operand_count(r, table, _loc())
    assert len(result) == 1
    assert result[0].code == "E009"


def test_xic_two_operands_emits_e009():
