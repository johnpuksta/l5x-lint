from l5x_lint.domain.checks.rll.er014_otl_without_otu import er014_otl_without_otu
...
def test_otl_with_otu_no_diagnostic():
    r = Routine(
        ...
        rll_rungs=[
            _make_rung(
                1, [Instruction(opcode="OTL", operands=[Operand(value="MyBit")])]
            ),
            _make_rung(
                2, [Instruction(opcode="OTU", operands=[Operand(value="MyBit")])]
            ),
        ],
    )
    diags = er014_otl_without_otu(
...
def test_multiple_otls_same_tag():
    r = Routine(
        ...
        rll_rungs=[
            _make_rung(
                1, [Instruction(opcode="OTL", operands=[Operand(value="BitA")])]
            ),
            _make_rung(
                2, [Instruction(opcode="OTL", operands=[Operand(value="BitA")])]
            ),
            _make_rung(
                3, [Instruction(opcode="OTU", operands=[Operand(value="BitA")])]
            ),
        ],
    )
    diags = er014_otl_without_otu(
...
def test_otl_and_otu_different_tags():
    r = Routine(
        ...
        rll_rungs=[
            ...
            _make_rung(
                2, [Instruction(opcode="OTU", operands=[Operand(value="BitB")])]
            ),
        ],
    )
    diags = er014_otl_without_otu(
